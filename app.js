...
//api 6
const forApisOutput = (tweetData, likesCount, replyCount) => {
  return {
...
app.get("/tweets/:tweetId/", authenticationToken, async (request, response) => {
  ...
  if (followingTweetIds.includes(parseInt(tweetId))) {
    ...
    response.send(forApisOutput(tweet_tweetDate, likes_count, reply_count));
  } else {
    response.status(401);
...
app.get("/user/tweets/", authenticationToken, async (request, response) => {
  let { username } = request;
  const getUserIdQuery = `select user_id from user where username IN ('${username}');`;
  const getUserId = await database.get(getUserIdQuery);
  console.log(getUserId);
  //get tweets made by user

  const getUserTweetIdsQuery = `SELECT tweet_id FROM TWEET WHERE user_Id IN (${getUserId.user_id});`;
  const getUserTweetIds = await database.all(getUserTweetIdsQuery);

  // get each tweetId

  const getTweetsId = getUserTweetIds.map((eachTweetId) => {
    return parseInt(eachTweetId.tweet_id);
  });

  // get each Tweet

  const getEachTweetQuery = `SELECT tweet FROM TWEET WHERE tweet_id IN (${getTweets});`;
  const getEachTweet = await database.all(getEachTweetQuery);
  const getEachTweetArray = getEachTweet.map((eachTweet) => {
    return eachTweet.tweet;
  });

  // likes count

  const likesCountQuery = `SELECT count(user_id) AS likes FROM LIKE WHERE tweet_id IN (${getTweetsId});`;
  const likes = await database.get(likesCountQuery);

  // replies count

  const repliesCountQuery = `SELECT count(user_id) AS replies FROM REPLY WHERE tweet_id IN (${getTweetsId});`;
  const replies = await database.get(repliesCountQuery);

  // dateTime

  const dateTimeQuery = `SELECT date_time  FROM TWEET WHERE tweet_id IN (${getTweetsId});`;
  const dateTime = await database.all(dateTimeQuery);

  response.send(forApisOutput(getEachTweet, likes, replies, dateTime));
});
